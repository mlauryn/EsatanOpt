...
class RU_MDP(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('gl', GLmtxComp(n=nn, GL_init=self.GL_init, user_links=self.conductors), promotes=['*'])
        #self.add_subsystem('gr', GRmtxComp(n=nn, GR_init=self.GR_init, faces=self.faces), promotes=['*'])
        self.add_subsystem('sol', Solar(npts=npts, areas=self.areas, nodes=self.nodes, model=self.model), promotes=['*'])
        self.add_subsystem('td', Thermal_direct(nn=nn, npts=npts, nodes=self.nodes), promotes=['*'])
